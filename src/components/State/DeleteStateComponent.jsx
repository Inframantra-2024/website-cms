...
import { deleteState, fetchStates } from '../../features/state/statesSlice';

const DeleteStateComponent = ({ stateId, closeModal }) => {
  const dispatch = useDispatch();
  const states1 = useSelector(state => state.states.data); // Adjust based on your state slice structure
 console.log(states1);
  ...
  const handleDeleteState = async () => {
    try {
      await dispatch(deleteState(selectedState._id)); // Adjust the id field based on your backend
      closeModal();
    } catch (error) {
  ...
  return (
    <div className="container mt-4">
      <h4 className='text-center bg-primary text-white p-3 text-uppercase'>Delete State</h4>
      <div className="mb-3">
...
export default DeleteStateComponent;
